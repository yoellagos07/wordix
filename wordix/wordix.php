...
function obtenerPuntajeWordix($palabraAdivinada, $intentosRealizados)  /* ****COMPLETAR***** parámetros formales necesarios */
{
    ...
    foreach (str_split('bcdfghjklm') as $letra) {
        $puntajesLetras[$letra] = 2;
    }

    foreach (str_split('npqrstvwxyz') as $letra) {
        $puntajesLetras[$letra] = 3;
    }

    // Calcula el puntaje por intento
    if ($intentosRealizados <= 6) {
        $puntaje = $puntajePorIntento[$intentosRealizados - 1];
    } else {
        $puntaje = 0;
    }
